...
1. **DONE 2026-09-23 (bug sweep): `system.touch`, the @MARKER TOUCH ATTACK branch in `rollWeaponAttack`.** **A touch roll for touch weapons.** Brok Harm Touch, Mephyt(Fire) Heat Skin and Mephyt(Ice) Cold
...
2. **BLOCKED 2026-09-23: the port tracks no lost limbs, so the function could never fire; see `2026-09-23-bug-sweep.md`.** **A lost limb loses its attack.** His `getNaturalAttackLost(name, disabilities, lostLimbs, race)`
...
3. **NEEDS A CALL (2026-09-23): title-scaled damage and rolled counts; see `2026-09-23-bug-sweep.md`.** **Famorian.** Its attacks come from its evokes (`setFamorianNaturalAttacks`). Extend
...
4. **DONE 2026-09-23 (bug sweep): "Add natural weapons" on the Equipment tab.** **A sheet button for characters made before this.** For now it is
...
6. **DONE 2026-09-23 (0.19.0).** **Release.** Bump `system.json` (0.19.0), move CHANGELOG's "Unreleased" under it, and run
