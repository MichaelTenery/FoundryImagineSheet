...
kept as he wrote it ("5 sec./AUR", "2`/AUR"). A spell's or invocation's formulas are worked out when
it is cast or invoked, by his own doSpellAction and doInvocationAction (section 4f). Schemas are in
`module/data/item-{consumable,lore,spell,invocation}.mjs`.
...
spell Item system:        # one row of his spellslist, 550 of them
  ...
  memorized    bool
  mastered     bool       # his spell_mastery_check: +2 Aura Control, half the time, x3 range, x2 duration,
                          # others resist at -20% (2026-09-24)
  days         number|null # his spell_days: 30 less the level when memorized (MEM, starting spells);
                          # one lost per Sleep; 0 while memorized is "forgotten", no cast. null = never
                          # counted, read as the full count while memorized (getSpellDaysLeft). Unticking
                          # Memorized writes 0 (2026-09-24)
  sourcebook, page, description
...
invocation Item system:   # one row of his invocationslist, 460 of them
  ...
  level, alignment, save, prayerTime, uses, invokeTime, range, area, duration, distance   # his columns, as text
                          # "uses" is uses per point of Piety Control, bought by a prayer
  memorized    bool
  usesLeft     number     # his invoc_uses: set by a prayer (uses x PC + .9, cut down), one spent per invoke;
                          # 0 when Memorized is unticked (his clearInvocUsages)
  sourcebook, page, description
...
Stored on the character, `system.magic` -- only what outlives a cast; everything else is derived:

```
magic:
  drainedAura       number   # his drained_aura: the pool is full less this
  additionalAura    number   # his additional_aura: Aura from items, on top of the attribute
  auraControlBoost  number   # his aura_control_boost: runes and items
  auraRegenDouble   bool     # his aura_regen_double
  magicSuppressed   bool     # his magic_suppression: Aura Control is 0, nothing is cast
  halfMagic         bool     # his half_magic: spells work at half the Aura put in (Aura Fatigue)
  pietyLevelBoost   number   # his piety_level_boost: added to Piety Control for invoking
  divineDenial      bool     # his divine_denial: no prayer, no invoking, Piety Control 0
  devotions         string   # his current_devotions, comma-separated ("General, Healing")
  ignoreDevotions   bool     # his ignore_devotions
  effectList        string   # his tmp_effect_list, for "Spell: X" / "Invoke: X" running on oneself
```

Derived in `_prepareMagic` (actor-character.mjs), after the skills, by `module/casting-rules.mjs`:
...
magic.piety:  isInvoker, control, level (control + boost), controlParts,
              divineKnowledge, bless, blasphemy
```

What a spell or invocation DOES is his own code, generated by `tools/extract/extract_casting.py`:
`module/casting-titles.mjs` (his per-class figures, read on every prepare) and
`module/casting-worker.mjs` (doSpellAction, doInvocationAction, the mishap table; loaded on the first
cast). `module/casting-helpers.mjs` stands in for everything of his that changed the caster, and
records it. The cast's chat card carries its attacks, targets and resistance rolls in
`flags.imagine-rpg.cast`, which its Apply buttons read -- and, for a mishap, `mishapEffects`
[{ kind: "aur" | "wil" | "suppress", value, label, applied }], each applied once: AUR and WIL onto the
attribute's `permMod`, Burnout onto `magic.magicSuppressed` (2026-09-24).
...
**The magic switches (2026-09-24): 21, one per repeating section of his.** arcane, divine, evoke,
powers, enchanting, divineItems, herbs, potions (and potion recipes), elixirs, poisons, charms,
ballads, hymns, poems, songs, candlelore, empathy, sympathy, glyphs, runes, rituals. `bardic` and
...
15. Magic ← class, title, Intelligence, Wisdom, Aura and the skills: Aura Control, the Aura Pool,
    regeneration, Piety Control (`_prepareMagic`, after the skills)
